...
public class UtilGrafos {
    ...
    public static boolean[][] obtenerMatrizCaminos(Map<Comparable, TVertice> vertices) {
        Double[][] matrizCostos=UtilGrafos.obtenerMatrizCostos(vertices);
        boolean[][] matrizCaminos=new boolean[vertices.size()][vertices.size()];
        for(int i=0;i<vertices.size();i++){
            for(int j=0;j<vertices.size();j++){
                if((matrizCostos[i][j]==Double.MAX_VALUE)||(matrizCostos[i][j]==-1d)){
                    matrizCaminos[i][j]=false;
                }
                else{
   ...
   public static <T extends IGrafoDirigido> T cargarGrafo(String nomArchVert, String nomArchAdy, 
    ...
    public static int posicionVertice(Map<Comparable, TVertice> vertices, Comparable buscar) {
        Set<Comparable> etiquetasVertices = vertices.keySet();
        Object[] VerticesIArr = etiquetasVertices.toArray();
        int cantidad = VerticesIArr.length;
        int posicion = 0;
        while (posicion < cantidad && !VerticesIArr[posicion].equals(buscar)) {
            posicion++;
        }
    ...
    static void imprimirMatriz(boolean[][] matriz, Map<Comparable, TVertice> vertices) {
        Object[] etiquetas = vertices.keySet().toArray();
        System.out.print("  ");
        for (int i = 0; i < matriz.length; i++) {
            System.out.print(etiquetas[i] + "  ");
