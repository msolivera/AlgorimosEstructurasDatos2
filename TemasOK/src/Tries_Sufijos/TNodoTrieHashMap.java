...
public class TNodoTrieHashMap {
    ...
    private TNodoTrieHashMap buscarNodoTrie(String s) {
     ...
     private void getPosicionesPatron(String patron, ArrayList<Integer> posiciones, TNodoTrieHashMap nodo) {
        if (nodo != null) {
            if (nodo.esPalabra) {
            ...
            for (Character caracter : nodo.hijos.keySet()){
                if (nodo.hijos.get(caracter) != null) {
                    nodo.hijos.get(caracter).getPosicionesPatron(patron, posiciones,nodo.hijos.get(caracter));
                }
            }
        }
    }

    public void getPosicionesPatron(String patron, ArrayList<Integer> posiciones) {
        TNodoTrieHashMap aux = this.buscarNodoTrie(patron);        
        getPosicionesPatron(patron, posiciones, aux);
    }
}
